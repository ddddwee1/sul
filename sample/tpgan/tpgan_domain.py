...
def gaussian_pyramid(inp):
	with tf.variable_scope('Gaussian_pyramid'):
		mod = M.Model(inp,[None,128,128,3])
		mod.pyrDown()#64
		mod.pyrDown()#32
		mod.pyrDown()#16
...
def build_total_graph():
	global totalLoss,dis_loss,train_d,train_g,train_e,updd,updg,upde,profHolder,gtHolder,leHolder,reHolder,mthHolder,nseHolder,domainHolder,zHolder,x_fake,losscollection
	...
	pyr_real = gaussian_pyramid(gtHolder)
	pyr_fake = gaussian_pyramid(x_fake)

	with tf.name_scope('pixel_loss'):
		pix_loss = tf.reduce_mean(tf.abs(pyr_real-pyr_fake))
	with tf.name_scope('sym_loss'):
		x_left, x_right = tf.split(pyr_fake,2,axis=2)
	...
	losscollection = [pix_loss,sym_loss,gen_loss,ip_loss,tv_loss,domain_loss]
	with tf.name_scope('Optimizer'):
		totalLoss = pix_loss+0.3*sym_loss+0.001*gen_loss+0.003*ip_loss+0.0001*tv_loss+0.1*domain_loss
