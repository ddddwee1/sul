...
class data_reader():
	...
	def get_train_batch(self,BSIZE):
		...
		# batch_age_fake = np.random.randint(7,size=BSIZE)
		# batch_age_fake = self.eye7[batch_age_fake]

		batch_age = np.float32(batch_age)
		batch_age_fake = np.float32(np.random.randint(15,70,size=[BSIZE,1]))
		return batch_img, batch_age_fake, batch_age