...
class Encoder(M.Model):
	def initialize(self):
		self.c1 = M.ConvLayer(5, 128, stride=2, activation=M.PARAM_LRELU)
		self.c2 = M.ConvLayer(5, 256, stride=2, activation=M.PARAM_LRELU)
		self.c3 = M.ConvLayer(5, 512, stride=2, activation=M.PARAM_LRELU)
		self.c4 = M.ConvLayer(5,1024, stride=2, activation=M.PARAM_LRELU)
...
class Decoder(M.Model):
	def initialize(self):
		...
		self.up1 = M.ConvLayer(3, 512*4, activation=M.PARAM_LRELU)
		self.up2 = M.ConvLayer(3, 256*4, activation=M.PARAM_LRELU)
		self.up3 = M.ConvLayer(3, 128*4, activation=M.PARAM_LRELU)
		self.up4 = M.ConvLayer(3, 64*4, activation=M.PARAM_LRELU)
		self.c5 = M.ConvLayer(5, 3, activation=M.PARAM_SIGMOID)
	def forward(self, x):
		x = self.fc1(x)
...
for i in range(MAX_ITER+1):
	...
	optim.apply_gradients(M.zip_grad(grad, swap_net.trainable_variables))
