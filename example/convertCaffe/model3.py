...
class Dense(KModel):
	def __init__(self, outsize, batch_norm=False, activation=-1 , usebias=True, norm=False, map_shape=None):
		super(Dense, self).__init__()
		self.fc = L.fcLayer(outsize, usebias, norm=norm, map_shape=map_shape)
		self.batch_norm = batch_norm
		self.activation = activation
		if batch_norm:
			self.bn = L.batch_norm()
