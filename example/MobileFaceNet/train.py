...
with tf.device('/cpu:0'):
	model = FaceResNet(data_reader.max_label + 1)
	optimizer = tf.optimizers.Adam(LR, 0.9)
	...
	pt = M.ParallelTraining(model, optimizer, [0,1,2,3,4,5,6,7], grad_loss_fn=grad_loss) 
	...
	for ep in range(EPOCH):
		for it in range(data_reader.iter_per_epoch):
			...
			lsttl = tf.reduce_mean([_[0] for _ in lsacc])
			ls = tf.reduce_mean([_[1] for _ in lsacc])
