...
def grad_loss(x, model):
	...
	with tf.GradientTape() as tape:
		out = model(data)
		ls = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels=lab, logits=out) * mask) / tf.reduce_sum(mask)
...
for it in range(config.max_iter + 1):
	...
	if it%10==0:
		...
		im_gt = visualize.draw_label(batch[1][0], batch[2][0])
		im_pred = visualize.draw_label(out[0].numpy(), batch[2][0])
