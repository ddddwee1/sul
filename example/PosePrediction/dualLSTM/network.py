...
class PosePredNet(M.Model):
	...
	def forward(self, enc_in, pred_step):
		...
		out_layer1 = [self.lstm_l1(vs[i], 0) for i in range(len(vs))]
		out_layer2a = [self.lstm_l2(x, 0) for x in out_layer1[0::2]]
		out_layer2b = [self.lstm_l2(x, 1) for x in out_layer1[1::2]]
