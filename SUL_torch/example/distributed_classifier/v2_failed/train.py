...
def initialize(batch, model):
	model.eval()
	batch = [i[0:4] if isinstance(i,torch.Tensor) else torch.from_numpy(i[0:4]) for i in batch]
	res = model(*batch)
	return res 
...
if __name__=='__main__':
	...
	# optim = torch.optim.SGD(arcface.parameters(), lr=0.01, momentum=0.9, weight_decay=0.0005)
	...
	for e in range(MAXEPOCH):
		for i in range(reader.iter_per_epoch):
			...
			optim.zero_grad()
			feats = model(imgs)
			loss = arcface(feats, labels, m1=1.0, m2=0.5, m3=0.0, s=64) 
			# t3 = time.time()
